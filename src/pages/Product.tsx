...
import { viewProductAPI } from "@/services2/operations/product"
...
export default function ProductPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      const data = await viewProductAPI(slug);
 ...
 const relatedProducts = products
  .filter(
    (p) =>
      p.slug !== product?.slug && p._id?.toString() !== product?._id?.toString()
  )
  .slice(0, 10);
  ...
  return (
    <>
      {
        product ? (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {product.images.map((image, index) => (
                      <button
                        ...
                        className={`relative flex-shrink-0 w-14 h-14 lg:w-16 lg:h-16 rounded-md lg:rounded-lg overflow-hidden transition-all duration-200 flex items-center justify-center ${selectedImageIndex === index
                    ...
                    {product.extraInfoBlocks.map((block, index) => (
                      ...
                      >
                        ...
                        <AccordionContent className="px-3 lg:px-6 pb-3 lg:pb-4 space-y-6 lg:space-y-8">
                          ...
                          >
                            <div
                              className={`space-y-2 lg:space-y-3 ${index % 2 === 1 ? "lg:order-2" : "lg:order-1"
                                }`}
                            >
                              <div
                                className="text-slate-700 dark:text-slate-300 text-xs lg:text-sm leading-relaxed"
